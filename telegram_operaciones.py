...
# aca va diccionario con tiempo de viaje por media hora y ruta, junto a polilinea de trazado de rutas fts del 104
...
criterio_spam_rorro = dt.timedelta(seconds=120)  # segundos para reconsultar la base de datos funcion rorro

seguro_telegramearlyexit = False  # esto sirve para que bot no auto-ejecute ultimo comando de salida al reiniciarlo
...
patentesL_electricos = ['LCTG-23', 'LCTG-24', 'LCTG-25', 'LCTG-26', 'LCTG-27', 'LCTG-28', 'LCTG-29', 'LCTG-30', 'LCTG-31', 'LCTG-32', 'LCTG-33',
                        'LCTG-34', 'LCTG-35', 'LCTG-36', 'LCTG-37', 'LCTG-38', 'LCTG-39', 'LCTG-40', 'LCTG-41', 'LCTG-42', 'LCTG-43', 'LCTG-44', 'LCTG-45', 'LCTG-46', 'LCTG-47']

# primeras nueve son para funciones originales f94_104 busesll busesep, sirven para reenviar mismo mensaje si se ha consultado en menos de criterio_spam segundos
...
# estas 3 hacen lo mismo que las 9 anteriores pero son un arreglo para que cada vez que se haga una nueva funcion solo haya que aumentar el tamano del arreglo
...
def sacar_tiempo_restante_macul(tiempo_mediahora, d, indice_min):
    if indice_min <= 176:
        return ((d[indice_min]-d[176])/(d[0]-d[176]))*tiempo_mediahora[0] + tiempo_mediahora[1] + tiempo_mediahora[2]
    elif indice_min <= 614:
        return ((d[indice_min]-d[614])/(d[176]-d[614]))*tiempo_mediahora[1] + tiempo_mediahora[2]
    else:
...
def ajustar_pulsos_a_ruta2(pulso, ruta_macul, ruta_vesp):
    # devuelve arreglo con numero indicando parte de la ruta en que va el bus, indice de polilinea que mejor ajusta pulso a ruta FTS
    ...
    if sur_de_metro_macul(pulso):
        ...
        indice_minimo_macul = min(range(len(distancias_pulso_ruta_macul)), key=distancias_pulso_ruta_macul.__getitem__)
    ...
    for t in ruta_macul:
        if contador_i < 579:  # partir buscando al norte de geocerca definida por sur_de_metro_macul()
            contador_i = contador_i + 1
        else:
            distancia_auxiliar = distance.distance(pulso, t).km
            if distancia_auxiliar < 0.020:
    ...
    if parar_busqueda:
        ...
        indice_minimo_vesp = min(range(len(distancias_pulso_ruta_vesp)), key=distancias_pulso_ruta_vesp.__getitem__)
        if distancias_pulso_ruta_macul[indice_minimo_macul] < distancias_pulso_ruta_vesp[indice_minimo_vesp] and distancias_pulso_ruta_macul[indice_minimo_macul] < 0.200:
...
# devuelve geocerca donde cae, la geocerca en ruta es una geocerca gigante que encierra varias comunas


def ubicacion(p):

    if p[1] >= -33.580831373999935 and p[1] <= -33.576390815942794 and p[2] >= -70.55463834229224 and p[2] <= -70.54917176737295:
        return "Cabezal Penon"
    elif p[1] >= -33.465386618998956 and p[1] <= -33.46095232045423 and p[2] >= -70.69815575321851 and p[2] <= -70.6926896346114:
        return "Cabezal 102"
    elif p[1] >= -33.42285028759169 and p[1] <= -33.41841188936765 and p[2] >= -70.61074963488507 and p[2] <= -70.60529056140658:
        return "Cabezal 104"
    elif p[1] >= -33.46246756 and p[1] <= -33.45802935 and p[2] >= -70.61256359 and p[2] <= -70.60710186:
        return "Cabezal 114"
    elif p[1] >= -33.562534 and p[1] <= -33.560964 and p[2] >= -70.558571 and p[1] <= ((-33.562534 - -33.560964)/(-70.555799 - -70.557356))*(p[2] - -70.557356) + -33.560964:
...
eq_geocerca_penon1_romboide = eq_geocerca_Romboide(coordenadas_geocerca_penon1[0], coordenadas_geocerca_penon1[1])

coordenadas_geocerca_penon2 = [[-33.578866, -70.5519543], [-33.577599, -70.55265]]
eq_geocerca_penon2_romboide = eq_geocerca_Romboide(coordenadas_geocerca_penon2[0], coordenadas_geocerca_penon2[1])

coordenadas_geocerca_losleones1 = [[-33.4208332, -70.6084863], [-33.4199595, -70.606523]]
eq_geocerca_losleones1_romboide = eq_geocerca_Romboide(
...
def geocerca_ElPenonDetenido_romboideX(p):
    if (p[0] - eq_geocerca_penon1_romboide[0]*p[1] - eq_geocerca_penon1_romboide[1] > 0) & (p[0] - eq_geocerca_penon1_romboide[0]*p[1] - eq_geocerca_penon1_romboide[1] < -1.0*eq_geocerca_penon1_romboide[0]*0.15) & (p[0] > eq_geocerca_penon1_romboide[2]) & (p[0] < eq_geocerca_penon1_romboide[3]):
        return True
    elif (p[0] - eq_geocerca_penon2_romboide[0]*p[1] - eq_geocerca_penon2_romboide[1] > 0) & (p[0] - eq_geocerca_penon2_romboide[0]*p[1] - eq_geocerca_penon2_romboide[1] < -1.0*eq_geocerca_penon2_romboide[0]*0.15) & (p[0] > eq_geocerca_penon2_romboide[2]) & (p[0] < eq_geocerca_penon2_romboide[3]):
        return True
    else:
        return False
...
def geocerca_LosLeonesDetenido_romboideX(p):
    if (p[0] - eq_geocerca_losleones1_romboide[0]*p[1] - eq_geocerca_losleones1_romboide[1] < 0) & (p[0] - eq_geocerca_losleones1_romboide[0]*p[1] - eq_geocerca_losleones1_romboide[1] > -1.0*eq_geocerca_losleones1_romboide[0]*0.15) & (p[0] > eq_geocerca_losleones1_romboide[2]) & (p[0] < eq_geocerca_losleones1_romboide[3]):
        return True
    else:
        return False
...
def dibujar_FTS(df_aux):
    m = folium.Map(location=[-33.5, -70.6], tiles='openstreetmap', zoom_start=12, control_scale=True)
    grp_pulsos = folium.FeatureGroup(name='Ultimos Pulsos GPS')
    for i in df_aux.index:
        popup_aux = df_aux.loc[i, 'PPU'] + " | " + diccionario_plaza_ppu[df_aux.loc[i, 'PPU']] + " | " + str(
            df_aux.loc[i, 'SSAB']) + " | " + df_aux.loc[i, 'SS_planillon'] + " | " + df_aux.loc[i, 'Ruta_Estimada'] + " | " + mensaje_tiempo_estimado(df_aux.loc[i, 'Tiempo_Viaje_Estimado'])
        folium.Marker([df_aux.loc[i, 'Lat'], df_aux.loc[i, 'Lon']], popup=popup_aux).add_to(grp_pulsos)
...
def dibujar_cabezal(df_aux, nombre_cabezal):
    ...
    def coordenadas_poligono_romboide_ejeX(latlon1, latlon2, pendiente_romboide, Ancho_geocerca_ejeX):
        return [latlon1, latlon2, [latlon2[0], latlon2[1]+Ancho_geocerca_ejeX], [latlon1[0], latlon1[1]+Ancho_geocerca_ejeX]]
    ...
    for i in df_aux.index:
        ...
        popup_aux = df_aux.loc[i, 'PPU'] + " | " + df_aux.loc[i, 'hora'].strftime(
            '%H:%M:%S') + " | " + diccionario_plaza_ppu[df_aux.loc[i, 'PPU']] + " | "+df_aux.loc[i, 'Detenido']
        folium.Marker([df_aux.loc[i, 'Lat'], df_aux.loc[i, 'Lon']], popup=popup_aux).add_to(grp_pulsos)

    if nombre_cabezal == "LosLeones":

        georcerca_losleones1 = coordenadas_poligono_romboide_ejeX(
            coordenadas_geocerca_losleones1[0], coordenadas_geocerca_losleones1[1], eq_geocerca_losleones1_romboide[0], 0.004*0.15)
        folium.Polygon(georcerca_losleones1, fill=False, color="purple", popup="LosLeonesDetenido").add_to(m)

    elif nombre_cabezal == "ElPenon":

        georcerca_penon1 = coordenadas_poligono_romboide_ejeX(
            coordenadas_geocerca_penon1[0], coordenadas_geocerca_penon1[1], eq_geocerca_penon1_romboide[0], 0.004*0.15)
        folium.Polygon(georcerca_penon1, fill=False, color="purple", popup="PenonDetenido1").add_to(m)
        georcerca_penon2 = coordenadas_poligono_romboide_ejeX(
            coordenadas_geocerca_penon2[0], coordenadas_geocerca_penon2[1], eq_geocerca_penon2_romboide[0], 0.004*0.15)
        folium.Polygon(georcerca_penon2, fill=False, color="purple", popup="PenonDetenido2").add_to(m)
...
def consultar_fts_104():
    ...
    if delta_query > criterio_spam or primera_queryf94_104:
        ...
        cur1.execute("SELECT patente, latitudgps, longitudgps, servicio, sentido, fecha, hora, servicio_sentido_a_bordo_del_bus, estado, velocidad_instantanea_del_bus, tiempo_detenido FROM ultimas_transmisiones;")
        datos = [row for row in cur1.fetchall() if row[0] in patentesL and row[1] != None and row[7][:3]
                 in servicios_validos_FTS and (not row[0] in patentesL_lostilos) and (not row[0] in patentesL_electricos)]
        ...
        datos_FTS = [row for row in datosOK if dt.datetime.combine(
            row[5], (dt.datetime.min + row[6]).time()) > ahora_delta and ubicacion(row) == 'En ruta']

        df = pd.DataFrame(datos_FTS, columns=['PPU', 'Lat', 'Lon', 'Sentido',
                                              'Servicio', 'fecha', 'hora', 'SSAB', 'Estado', 'V_inst', 'T_detenido'])
        ...
        if df.empty:
            ...
            df['Media_hra'] = df.apply(lambda x: x['hora'] - pd.Timedelta(minutes=(x['hora'].minute %
                                                                                   30), seconds=x['hora'].second), axis=1).dt.strftime("%H:%M")

            #df['dX'] = 999
            #df.loc[df['Sentido_SSAB']=='R','dX'] = df.loc[df['Sentido_SSAB']=='R'].apply(lambda x: distance.distance((x['Lat'], x['Lon']), (LosLeones_centroide[0], LosLeones_centroide[1])).km, axis = 1)
            #df.loc[df['Sentido_SSAB']=='I','dX'] = df.loc[df['Sentido_SSAB']=='I'].apply(lambda x: distance.distance((x['Lat'], x['Lon']), (ElPennon_centroide[0], ElPennon_centroide[1])).km, axis = 1)
            ...
            query2 = "SELECT ppu, fecha, horaprogramada, horasalidareal, servicio, sentido, conductor_nombre FROM planillon_despachos WHERE fts = 'S' AND fecha >= '" + \
                fecha_limite + "' AND horaprogramada >= '" + hora_limite + "' AND ppu in "+ppus_encontradas+";"
            cur2.execute(query2)
            datos_planillon = [row for row in cur2.fetchall()]

            df_planillon = pd.DataFrame(datos_planillon, columns=[
                                        'PPU', 'Fecha_planillon', 'Hora_Programada_planillon', 'Hora_Salida_Real_planillon', 'Servicio_planillon', 'Sentido_planillon', 'Nombre_Conductor'])
            df_planillon['SS_planillon'] = df_planillon['Servicio_planillon'] + df_planillon['Sentido_planillon']
            ...
            for i in df.index:
                ...
                ajuste_a_pulso = ajustar_pulsos_a_ruta2(
                    [df.loc[i, 'Lat'], df.loc[i, 'Lon']], ruta_fts104_10metros['104_FTS_R_Macul']['polilinea'], ruta_fts104_10metros['104_FTS_R_Vesp']['polilinea'])
                ...
                if ajuste_a_pulso[0] >= 11 and ajuste_a_pulso[0] <= 19:  # usar polilinea macul
                    df.loc[i, 'Tiempo_Viaje_Estimado'] = sacar_tiempo_restante_macul(
                        ruta_fts104_10metros['Tiempos_de_viaje'][df.loc[i, 'Media_hra']], ruta_fts104_10metros['104_FTS_R_Macul']['dist_acum'], ajuste_a_pulso[1])
                    ...
                    df.loc[i, 'Tiempo_Viaje_Estimado'] = sacar_tiempo_restante_vespucio(
                        ruta_fts104_10metros['Tiempos_de_viaje'][df.loc[i, 'Media_hra']], ruta_fts104_10metros['104_FTS_R_Vesp']['dist_acum'], ajuste_a_pulso[1])
            ...
            for i in df.index:
                ...
                mensaje_telegram = mensaje_telegram + df.loc[i, 'PPU'] + "  |    " + diccionario_plaza_ppu[df.loc[i, 'PPU']] + "    | " + df.loc[i,
                                                                                                                                                 'SS_planillon'] + " | " + df.loc[i, 'Ruta_Estimada'] + " |  " + mensaje_tiempo_estimado(df.loc[i, 'Tiempo_Viaje_Estimado']) + "\n"
...
def consultar_buses_cabezal_LosLeones():
    ...
    if delta_query > criterio_spam or primera_querybusesll:
        ...
        cur1.execute("SELECT patente, latitudgps, longitudgps, servicio, sentido, fecha, hora, servicio_sentido_a_bordo_del_bus, estado, velocidad_instantanea_del_bus, tiempo_detenido FROM ultimas_transmisiones;")
        datos = [row for row in cur1.fetchall() if row[0] in patentesL and row[1] != None and (
            not row[0] in patentesL_lostilos) and (not row[0] in patentesL_electricos)]
        datosOK = [[row[0], float(row[1]), float(row[2]), row[3], row[4], row[5], row[6],
                    row[7], row[8], int(row[9]), int(row[10])] for row in datos]
        datos_cabezal = [row for row in datosOK if dt.datetime.combine(
            row[5], (dt.datetime.min + row[6]).time()) > ahora_delta and ubicacion(row) == "Cabezal 104"]
        datos_cabezal = [[row[0], row[1], row[2], row[6], row[8], int(row[10])] for row in datos_cabezal]

        df = pd.DataFrame(datos_cabezal, columns=['PPU', 'Lat', 'Lon', 'hora', 'Estado', 'T_detenido'])
        ...
        if df.empty:
            ...
            if len(df2.index) == 1:
                ...
                mensaje_telegram = "Hay " + \
                    str(len(df.index)) + " buses cerca de cabezal Los Leones, " + str(len(df2.index)) + " detenidos\n"

            mensaje_telegram = mensaje_telegram + "Patente | Hora GPS | Plazas | Detenido\n"

            for i in df.index:
                mensaje_telegram = mensaje_telegram + df.loc[i, 'PPU'] + "  |    " + df.loc[i, 'hora'].strftime(
                    '%H:%M:%S') + "    |   " + diccionario_plaza_ppu[df.loc[i, 'PPU']] + "  | "+df.loc[i, 'Detenido']+"\n"
...
def consultar_buses_cabezal_ElPenon():
    ...
    if delta_query > criterio_spam or primera_querybusesep:
        ...
        datosOK = [[row[0], float(row[1]), float(row[2]), row[3], row[4], row[5], row[6],
                    row[7], row[8], int(row[9]), int(row[10])] for row in datos]
        datos_cabezal = [row for row in datosOK if dt.datetime.combine(
            row[5], (dt.datetime.min + row[6]).time()) > ahora_delta and ubicacion(row) == "Cabezal Penon"]
        datos_cabezal = [[row[0], row[1], row[2], row[6], row[8], int(row[10])] for row in datos_cabezal]
        ...
        if df.empty:
            ...
            if len(df2.index) == 1:
                ...
                mensaje_telegram = "Hay " + \
                    str(len(df.index)) + " buses cerca de cabezal El Penon, " + str(len(df2.index)) + " detenidos\n"
...
def consultar_rorro():
    ...
    if delta_query > criterio_spam_rorro or primera_query_arreglo[numero_de_funcion_query]:
        ...
        cur1 = db1.cursor()
        cur1.execute("SELECT patente, latitudgps, longitudgps, servicio, sentido, fecha, hora, servicio_sentido_a_bordo_del_bus, estado, velocidad_instantanea_del_bus, tiempo_detenido, idwebservice FROM ultimas_transmisiones;")

        datos = [row for row in cur1.fetchall() if len(row) == 12 and row[1] != None]
        ...
        datos_cabezal = [[row[0], row[1], row[2], row[6], row[8], row[10], row[11]] for row in datos_cabezal]

        if datos_cabezal:
            datos_cabezal = [datos_cabezal[randint(0, (len(datos_cabezal)-1))]]
            df = pd.DataFrame(datos_cabezal, columns=['PPU', 'Lat', 'Lon',
                                                      'hora', 'Estado', 'T_detenido', 'idwebservice'])
        ...
        if df.empty:
            ...
            cur3 = db3.cursor()
            cur3.execute("SELECT estado_ignicion FROM ws_pos_dia_2019 WHERE id = " + df.loc[i, 'idwebservice'])

            df.loc[i, 'estado_ignicion'] = cur3.fetchall()[0][0]

            mensaje_telegram = "Patente | Estado Ignicion | Hora\n"
            mensaje_telegram = mensaje_telegram + df.loc[i, 'PPU'] + "  |  " + df.loc[i,
                                                                                      'estado_ignicion'] + "   | " + df.loc[i, 'hora'].strftime('%H:%M:%S') + "\n"
...
def consultar_ultima_transmision_10():
    ...
    if delta_query > criterio_spam or primera_query_arreglo[numero_de_funcion_query]:
        ...
        cur1.execute("SELECT patente, latitudgps, longitudgps, servicio, sentido, fecha, hora, servicio_sentido_a_bordo_del_bus, estado, velocidad_instantanea_del_bus, tiempo_detenido, idwebservice, ubicacion FROM ultimas_transmisiones;")

        datos = [row for row in cur1.fetchall()]
        datos = [datos[i] for i in sample(range(0, (len(datos)-1)), tamanno_muestra)]
        datosOK = [[row[0], float(row[1]), float(row[2]), row[3], row[4], row[5], row[6], row[7], row[8], int(
            row[9]), int(row[10]), str(row[11]), str(row[12])] for row in datos]
        datos_cabezal = [[row[0], row[1], row[2], row[6], row[8], row[10], row[11], row[12]] for row in datosOK]

        df = pd.DataFrame(datos_cabezal, columns=['PPU', 'Lat', 'Lon', 'hora',
                                                  'Estado', 'T_detenido', 'idwebservice', 'ubicacion'])
        ...
        if df.empty:
            ...
            mensaje_telegram = "Se sacó una muestra de " + \
                str(len(df.index)) + " buses entre las últimas transmisiones\n"
            mensaje_telegram = mensaje_telegram + "Patente |   Hora   | Ubicación\n"

            for i in df.index:
                mensaje_telegram = mensaje_telegram + \
                    df.loc[i, 'PPU'] + " | " + \
                    df.loc[i, 'hora'].strftime('%H:%M:%S') + " | " + df.loc[i, 'ubicacion'] + "\n"
...
def consultar_ultima_transmision_electricos():
    ...
    if delta_query > criterio_spam or primera_query_arreglo[numero_de_funcion_query]:
        ...
        datos = [row for row in cur1.fetchall() if row[0] in patentesL_electricos and row[1] != None]
        datosOK = [[row[0], float(row[1]), float(row[2]), row[3], row[4], row[5], row[6], row[7], row[8], int(
            row[9]), int(row[10]), str(row[11]), str(row[12])] for row in datos]
        datos_cabezal = [[row[0], row[1], row[2], row[6], row[7], row[8], row[10], row[11], row[12]]
                         for row in datosOK]

        df = pd.DataFrame(datos_cabezal, columns=['PPU', 'Lat', 'Lon', 'hora',
                                                  'SSAB', 'Estado', 'T_detenido', 'idwebservice', 'ubicacion'])
        ...
        if df.empty:
            ...
            for i in df.index:
                mensaje_telegram = mensaje_telegram + df.loc[i, 'PPU'] + " | " + df.loc[i, 'hora'].strftime(
                    '%H:%M:%S') + " | " + df.loc[i, 'SS'] + " | " + df.loc[i, 'ubicacion'] + "\n"
...
def consultar_patentes_ultima_transmision_maipu():
    ...
    if delta_query > criterio_spam or primera_query_arreglo[numero_de_funcion_query]:
        ...
        cur1.execute("SELECT patente, latitudgps, longitudgps, servicio, sentido, fecha, hora, servicio_sentido_a_bordo_del_bus, estado, velocidad_instantanea_del_bus, tiempo_detenido, idwebservice, ubicacion FROM ultimas_transmisiones_c;")
        ...
        if df.empty:
            ...
            mensaje_telegram = "Hay " + \
                str(len(df.index)) + " patentes distintas entre las últimas transmisiones\n"
...
def consultar_anexo3(servicio="F01", sentido="Ida", tipo_dia="Laboral", periodo="ts"):
    ...
    if servicio in servicios_STP and sentido in ['Ida', 'Ret'] and tipo_dia in ['Laboral', 'Sábado', 'Domingo'] and periodo in ['mh', 'ts']:
        ...
        if periodo == 'mh':
            ...
            for q in query_anexo3.index:
                mensaje_telegram = mensaje_telegram + query_anexo3.loc[q, 'MH'].strftime('%H:%M') + " |   " + str(
                    query_anexo3.loc[q, 'N° Salidas']) + "     |  " + str(query_anexo3.loc[q, 'Capacidad (plazas)']) + "\n"
        else:
            mensaje_telegram = mensaje_telegram + "PeriodoTS | Salidas\n"
            query_anexo3_procesada = query_anexo3.groupby(['Periodo'])[
                'Velocidad (Km/hra)', 'Distancia Base (Km)', 'Distancia Total (POB+POI) (Km)', 'Tiempo recorrido (hra)', 'Capacidad (plazas)'].transform('mean')
            ...
            for q in query_anexo3_procesada.index:
                ...
                mensaje_telegram = mensaje_telegram + \
                    str(query_anexo3_procesada.loc[q, 'Periodo']) + string_columna + \
                    str(query_anexo3_procesada.loc[q, 'Salidas_periodo']) + "\n"

        mensaje_telegram = mensaje_telegram + " \nTotal salidas:    " + str(query_anexo3['N° Salidas'].sum()) + "\n"
...
def procesar_argumento_comando_anexo3(argumentos):
    ...
    if argumentos:
        ...
        if argumentos[0].upper() in servicios_STP:
            ...
            interpretacion = "Ese no me lo sabía, servicio tiene que estar con código transantiago, ejemplos: F53e, F52N, F01c, F02"
    else:
        interpretacion = "Intentalo de nuevo. Escribe un servicio y alguna de las opciones en paréntesis [ ]: /anexo3 codigo_servicio_TS [i,r] [l,s,d] [mh,ts]"

    if servicio_i:
        interpretacion = "Asumiendo que quisiste decir:\n /anexo3 " + \
            servicio_i + " " + sentido_i+" "+tipo_dia_i+" " + periodio_i
...
def consultar_donde_esta_ppu(ppu_q=""):
    ...
    cur1.execute("SELECT patente, latitudgps, longitudgps, servicio, sentido, fecha, hora, servicio_sentido_a_bordo_del_bus, estado, ubicacion, lugar, estado_ignicion  FROM ultimas_transmisiones WHERE patente = '" + q_ppu + "' ;")
...
def consultar_donde_esta_ppu_maipu(ppu_q=""):
    ...
    cur1.execute("SELECT patente, latitudgps, longitudgps, servicio, sentido, fecha, hora, servicio_sentido_a_bordo_del_bus, estado, ubicacion, lugar, estado_ignicion  FROM ultimas_transmisiones_c WHERE patente = '" + q_ppu + "' ;")
...
def start(bot, update):
    bot.send_message(chat_id=update.message.chat_id,
                     text="Hola, para empezar a usar este bot debes saber la password. Si tienes dudas sobre cómo ocuparlo mándame el mensaje /ayuda")


def ayuda(bot, update):
    if str(update.effective_user.id) in lista_acceso_dic:
        bot.send_message(chat_id=update.message.chat_id, text="Este bot (version " + n_version + ") toma datos GPS y del planillón online para entregar información en tiempo real sobre los buses de STP, además puede entregar información del Plan Operacional. Para consultar puedes enviar:\n" +
                         "/comandos - enlista todos los comandos posibles\n" +
                         "/version - dice qué versión del bot está siendo usada\n" +
                         "/f94_104 - dice en cuantos minutos llegan los F94 hacia metro Los Leones. En caso que un bus se encuentre fuera de las dos rutas posibles, Vespucio-Tobalaba y Macul-Leones, no se estima el tiempo y se pone 'FR'\n" +
                         "/busesLL - dice cuantos buses se encuentran muy cerca del cabezal Metro Los Leones y cuáles se encuentran detenidos\n" +
                         "/busesEP - dice cuantos buses se encuentran muy cerca del cabezal El Peñón y cuáles se encuentran detenidos\n" +
                         "NOTA: En caso de que un bus no se ingresó al Planillón Online, no se puede saber para cual servicio es el F94, de todas formas se muestran estos buses con un 'NA' en la columna Servicio\n" +
                         "/uGPS_Electricos - dice patentes de los 25 buses Electricos, la hora de su última transmisión GPS y sus columnas 'servicio_sentido_a_bordo_del_bus' (abreviada) y 'Ubicación' del webservice\n" +
                         "/uGPS_10 - dice patentes de 10 buses al azar, la hora de su última transmisión GPS y su columna Ubicación del webservice\n" +
                         "/patentes_maipu - dice cuantas iniciales de patentes hay transmitiendo en la tabla de últimas transmisiones\n" +
                         "/anexo3 - dice salidas según anexo 3, por ejemplo '/anexo3 F53e i l mh' consulta salidas por media hora(mh) día laboral(l) sentido ida(i) del F53e\n" +
                         "/donde - dice donde está una PPU y entrega columnas webservice (servicio-sentido-fecha-hora-SSAB-estado-ubicacion), ejemplo /donde FLXT33\n" +
                         "/donde_maipu - dice donde está una PPU del terminal de Maipú, ejemplo /donde BBZX38")

    else:
        bot.send_message(chat_id=update.message.chat_id,
                         text="No tienes permiso para usar el bot, habla con alguien de Operaciones para pedir ayuda sobre cómo tener acceso.")
...
def ayuda_nuevo_acceso(bot, update):
    if str(update.effective_user.id) == id_bruno_stefoni:
        texto = ('t.me/STPOperacionesBOT' + '\n' + 'Con telegram instalado ese link abre un chat al bot de operaciones' + '\n' +
                 'Para poder consultar debes registrar una cuenta (con algún NOMBRE, por ejemplo: Diego), solo basta con enviarle el siguiente texto al bot:' + '\n' +
                 '"/manzana roja NOMBRE"' + '\n' +
                 'Por ejemplo:' + '\n' +
                 '/manzana roja Diego' + '\n' +
        ...
        if args:
            if len(args[0].strip()) == 6:
                ...
                if latlon_consulta:
                    ...
                    bot.send_message(chat_id=update.message.chat_id, text="No se encontró la PPU " +
                                     args[0].strip().upper() + " quizá la PPU no existe o falló la conexión")
            else:
                bot.send_message(chat_id=update.message.chat_id,
                                 text="Tiene que ser una ppu sin guiones, por ejemplo /donde FLXT33")
        else:
            bot.send_message(chat_id=update.message.chat_id,
                             text="Enviar una ppu sin guiones con el comando, por ejemplo /donde FLXT33")
    else:
        bot.send_message(chat_id=update.message.chat_id, text="Acceso denegado.")
    ...
    if str(update.effective_user.id) in lista_acceso_dic:
        if lista_acceso_dic[str(update.effective_user.id)]:
            if lista_acceso_dic[str(update.effective_user.id)][0].upper() == 'R':
                bot.send_message(chat_id=update.message.chat_id, text="Hola 🐽 Rorro, descartando datos..")
...
def reset_accesos(bot, update, args):
    if args:
        if str(update.effective_user.id) == id_bruno_stefoni:
            ...
            bot.send_message(chat_id=update.message.chat_id, text="Hola Bruno, se reseteó el diccionario con accesos")
        else:
            bot.send_message(chat_id=update.message.chat_id, text="Solo Bruno puede resetear accesos")
...
def guardar_accesos(bot, update, args):
    if args:
        if str(update.effective_user.id) == id_bruno_stefoni:
            ...
            outfile.close()
            bot.send_message(chat_id=update.message.chat_id,
                             text="Hola Bruno, se guardó el diccionario con los accesos actualizados")
        else:
            bot.send_message(chat_id=update.message.chat_id, text="Solo Bruno puede guardar los accesos actuales")
        ...
        if args[0].strip().lower() == "roja":
            if str(update.effective_user.id) in lista_acceso_dic:
                ...
                bot.send_message(chat_id=update.message.chat_id, text="Hola " +
                                 lista_acceso_dic[str(update.effective_user.id)] + " voy a intentar cambiar tu nombre..")
            else:
                bot.send_message(chat_id=update.message.chat_id, text="No te conocía, agregando a lista de acceso..")
            ...
            if len(args) > 1:
                if len(args) < 7:
                    ...
                    if len(argumentos) < 60:
                        ...
                        bot.send_message(chat_id=update.message.chat_id, text="Te agregué con el nombre " + argumentos)
                    else:
                        bot.send_message(chat_id=update.message.chat_id,
                                         text="No te agregué con el nombre dado, debes poner menos de 60 caracteres")
                else:
                    bot.send_message(chat_id=update.message.chat_id,
                                     text="No te agregué con el nombre dado, debes poner menos de 6 palabras")
            else:
                bot.send_message(chat_id=update.message.chat_id,
                                 text="Te agregué aunque no me diste ningún nombre, si quieres intenta de nuevo con\n/manzana roja Nombre")
